...
const SignUpForm = () => {
  ...
  //Todo: WE Can set specific message on on specific fields 
  const [focus, setUserFocus] = React.useState(false);
  ...
  const handleSubmit = async (e) => {
     ...
     if (user.password !== user.conFirmPassword) {
      setProcessing(false);
        return console.log("Dont Proceed");
     }
     try {
  ...
  return (
    ...
    <div className='w-full text-end' >{processing ? <p>Siging up...</p> : null}</div>
          ...
          <div className="p-5 space-y-4 md:space-y-6 ">
            ...
            <div>
              ...
              <div>
                ...
                <input type="password" name="confirm-password" id="confirm-password" onFocus={() => setUserFocus(true)}
                    onBlur={() => setUserFocus(false)}  onChange={(e) => setUser({...user, conFirmPassword: e.target.value})} placeholder="••••••••" className="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-1.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required />
                   <div>{user.password !== user.conFirmPassword && focus ? <span>Must Match the Password and Field</span>: null}</div>
              </div>
              <button disabled={!user.email || !user.password ? true : false } onClick={handleSubmit} type="submit" className="w-full text-white bg-cyan-600 hover:bg-cyan-700 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">Create an account</button>
